...
class KeyboardExtensionAuditTests(unittest.TestCase):
    def test_inspects_installable_keyboard_extension_inside_ipa(self):
        ...
        self.assertIn('PlugIns/GeflechtBoardKeyboard.appex', audit)
        self.assertIn('Print :CFBundleExecutable', audit)
        self.assertIn('"$extension_path/$extension_executable"', audit)
        self.assertIn('otool -hv', audit)
